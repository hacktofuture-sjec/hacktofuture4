...
from ingestion.github_scraper import scrape_github_issues
from ingestion.web_scraper import scrape_nitter, scrape_reddit, scrape_hackernews
...
def get_stats():
    ...
    try:
        ...
        return {
            ...
            "feedback_by_source": {
                "github": github_count.count or 0,
                "reddit": reddit_count.count or 0,
            },
            "learnings": {
...
def ingest(req: IngestRequest, background_tasks: BackgroundTasks):
    ...
    def _ingest():
        try:
            items = []
            query_terms = [t.lower() for t in req.search_query.split() if t.strip()]

            # GitHub Issues
            if req.include_github and req.repo_name:
                items += scrape_github_issues(req.repo_name, req.max_github_issues)

            # Reddit
            if req.include_reddit:
                items += scrape_reddit(req.reddit_subreddit, req.search_query, req.max_reddit_posts)

            # HackerNews
            if req.include_hackernews:
                items += scrape_hackernews(req.search_query)

            # Twitter/Nitter
            if req.include_twitter:
                items += scrape_nitter(req.search_query)

            # Optional strict filter: keep only items containing all query terms.
            if req.strict_query_match and query_terms:
                items = [
                    i for i in items
                    if all(term in (i.get("text") or "").lower() for term in query_terms)
                ]

            count = save_feedback(items)
            embed_feedback()
            cluster_feedback()
            logger.info(f"Ingest complete: {count} new items")
        except Exception as e:
