...
def api_upload(request):
    if request.method == 'POST':
        ...
        if video_upload_form.is_valid():
            ...
            if not settings.DEBUG:
                upload_dir = os.path.join(settings.PROJECT_DIR, 'uploaded_videos', 'app', 'uploaded_videos')
                
            os.makedirs(upload_dir, exist_ok=True)
            file_path = os.path.join(upload_dir, saved_video_file)
            ...
            return JsonResponse({"status": "success", "file_name": saved_video_file})
        else:
...
def api_predict(request):
    if request.method == 'POST':
        ...
        def extract_frames_internal(video_path: str):
            try:
                pipeline_state['dataset'] = validation_dataset([video_path], sequence_length=sequence_length, transform=train_transforms)
                ...
                for i in range(sequence_length):
                    ...
                    if frame_face.size > 0:
                        ...
                        img_face_rgb.save(image_path)
                        faces_found += 1
                        pipeline_state['faces_cropped_images'].append(image_name)

                pipeline_state['faces_found'] = faces_found
                return faces_found
            except Exception as e:
        ...
        # Extract frames
        faces_found = extract_frames_internal(video_file)
        
        if faces_found == 0:
            return JsonResponse({"error": "No faces found in video"}, status=400)
        ...
        try:
            ...
            return JsonResponse({
                ...
                "preprocessed_images": pipeline_state['preprocessed_images'],
                "faces_cropped_images": pipeline_state['faces_cropped_images'],
